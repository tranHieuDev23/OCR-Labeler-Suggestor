...
class Predictor():
    ...
    def predict_batch(self, batch_img):
        batch_img = process_batch_input(batch_img, self.config['dataset']['image_height'],
                                        self.config['dataset']['image_min_width'], self.config['dataset']['image_max_width'])
    ...
    def predict_with_boxes(self, img, boxes):
        ...
        for box in boxes:
            ...
            if l1 != l2 or t1 != t2 or r1 != r2 or b1 != b2:
                # polyline : transform
                crop_transform_img = self.transform_image(
                    np.array(img), np.array(box, dtype='float32'))
